...
// 处理注册
const useRegisterEffect = (showToast,handlePush) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const result = await post("/api/users/register", registerMessage);
      if (result?.errno === 0) {
        showToast("注册成功!");
        setTimeout(() => {
          handlePush("Login",{ username: result?.data.username, password: result?.data.password });
        }, 800);
      } else {
        showToast("注册失败!");
      }
...
export default {
  ...
  setup() {
    ...
    const { username, password, ensurement, handleRegister } =
      useRegisterEffect(showToast,handlePush);
