...
<script>
import { useRoute } from "vue-router"; // 路由插件
...
// 处理登录
const useLoginEffect = (showToast, handlePush) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const result = await post("/api/users/login", loginMessage);
      if (result?.errno === 0) {
        showToast("登录成功!");
        localStorage.token = result?.token
        localStorage.isExpire = false;
        setTimeout(() => {
          handlePush("Home");
        }, 800);
      } else {
...
export default {
  ...
  setup() {
    ...
    let { username, password, handleLogin } = useLoginEffect(
      showToast,
      handlePush
    );
